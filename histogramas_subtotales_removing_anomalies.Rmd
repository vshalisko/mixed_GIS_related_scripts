...
### Script para visualización de las histogramas y calculo de las estadisticas descriptivas por clase de la variable categorica, variante con exclusión de os valores atípicos.
...
my_path <- "C:\\imeplan\\Estadisticas_R\\Poblacion No Residente"
my_filename <- 'cuadros_1ha_indice_de_habitantes_por_viviendal.dbf'
my_out_statistics_table <- 'cuadros_1ha_indice_de_habitantes_removing_anomalies_por_viviendal_statistics.csv'
my_out_histogram_table <- 'cuadros_1ha_indice_de_habitantes_por_viviendal_removing_anomalies_histogramas.csv'

my_category_field = "Cve_mun"
my_interest_field = "In_De_Viv"

my_breaks <- c(0,0.5,1,1.5,2,2.5,3,3.5,4,4.5,5,10)
my_title = "habitantes por vivenda por 1 Ha"
...
my_quantile_total_init <- as.vector(quantile(my_data[[my_interest_field]],na.rm = TRUE))

# excluding anomalous data (data oustide of 3 IQR counted from Q1 & Q3)
my_anomal_data_indicator <- (( my_data[,my_interest_field] > my_quantile_total_init[4] + 
                               3 * (my_quantile_total_init[4] - my_quantile_total_init[2]) ) | 
                               ( my_data[,my_interest_field] < my_quantile_total_init[2] - 
                               3 * (my_quantile_total_init[4] - my_quantile_total_init[2])) )


summary(my_anomal_data_indicator)

my_data <- my_data[!my_anomal_data_indicator,]

# summary of data without anomalous values
dim(my_data)
summary(my_data)
